...
using PolarDB;
    ...
    public class IndexByOrderedSet<T> : IndexByOrderedSetBase where T : IComparable<T>
    {
        ...
        public IEnumerable<long> SearchAllOffsets(T key)
        {
        ...
        public PaEntry SearchOneOffset(T key)
        {
            if (addCache.TryGetValue(key, out hasheSet))
            {
            var offsetsEntry = offsets.Root.Element(0);
                offsetsEntry.offset = hasheSet.First();
                return offsetsEntry;
            }

            if (table.IsEmpty || table.Count() == 0) return PaEntry.Empty;
            ptr = table.Element(0);
            return  offsets.Root.BinarySearchFirst(entry =>
            {
                ptr.offset = (long) entry.Get();
                return keyProducer(ptr).CompareTo(key);
            });
        ...
        public IEnumerable<object> SearchAll(T row)
        {
            if (table.IsEmpty || table.Count() == 0) return Enumerable.Empty<object>();
            ptr = table.Element(0);
            return SearchAllOffsets(row).Select(offset =>
            {
                ptr.offset = offset;
                return ptr.Get();
            });
        }
        public object SearchOne(T row)
        {
            if (table.IsEmpty || table.Count() == 0) return Enumerable.Empty<object>();
            ptr = table.Element(0);
            var entry = SearchOneOffset(row);
            if(entry.IsEmpty)  return null;

            {
                ptr.offset = (long) entry.Field(0).Get();
                return ptr.Get();
            }
        }
    ...
    public class IndexByOrderedSet<T1,T2> :IndexByOrderedSetBase where T1 : IComparable
        where T2 : IComparable
    {
        ...
        public void Build()
        {
            ...
            offsets.Flush();
            using (StreamWriter w=new StreamWriter(@"C:\Temp\test.txt",false))
            foreach (var element in offsets.Root.Elements())
            {
                ptr.offset = (long)element.Get();
                w.WriteLine(((object[])ptr.Get())[1]);
                w.WriteLine(((object[])ptr.Get())[2]);
                w.WriteLine("____________________             ________________");
                w.WriteLine(key1Producer(ptr));
                var comparableObject = key2Producer(ptr) as ComparableObject;
                if (comparableObject == null) break;
                w.WriteLine(comparableObject.tag);
                w.WriteLine(comparableObject.uriCode);
                w.WriteLine(comparableObject.Content);
                w.WriteLine("____________________________________");
            }
            offsets.Root.SortByKey(offset =>
            {
                ptr.offset = (long) offset;
