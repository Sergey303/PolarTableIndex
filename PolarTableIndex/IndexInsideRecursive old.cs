...
{
    public class IndexInsideRecursive : PaCell, IIndex<dynamic[]>
    {
                ...
                dirPath + "\\index.pac", false)
        {
            ...
            if (rowTypes.Length > 0)
            {
                ...
                minCombinedkey = Combine(((object[])Root.Element(0).Get()).Select(o1 => (dynamic)Convert.ChangeType(o1, rowTypes[0])));
                maxCombinedkey = Combine(((object[])Root.Element(Root.Count() - 1).Get()).Select(o1 => (dynamic)Convert.ChangeType(o1, rowTypes[0])));

                    Root.Scan(o =>
                    {
                        var subKey = GetSubKey(Combine(((object[]) o).Select(o1 => (dynamic)Convert.ChangeType(o1, rowTypes[0]))));
        ...
        { 
            ...
            scan((offset, obj) =>
            {
                ...
                var combined = Combine(keyF.Select(f => f(objects)));
       ...
       /// <returns></returns>
        private BigInteger Combine(IEnumerable<dynamic> objects)
        {
            return objects
        ...
        public IEnumerable<long> GetRowsOffsetsByKey(dynamic[] keys)
        {
            ...
            if (diapason.numb >= maxDiapason4Binarysearch)
                ...
                return Root.Elements(diapason.start, diapason.numb)
                    .Select(entry => ((object[]) entry.Get())TODO.ToArray())
                    .SkipWhile(
                        row =>
                            CompareRows(keys,
