...
namespace PolarTableIndex
{
    class FirstSimple
    {
        ...
        //}
        private static void FirstSimpleExperiment()
        {
            ...
            index =
                new PaCell(
                    new PTypeSequence(new PTypeRecord(new NamedType("hkey", new PType(PTypeEnumeration.integer)),
                        new NamedType("offset", new PType(PTypeEnumeration.longinteger)))), "../../index.pac", false);
            ...
            timer.Stop();
            Console.WriteLine("indexSeachByName " + timer.Elapsed.TotalMilliseconds + "ms " +
                              indexSeachByName.Field(1).Get().ToString());
        }

        private static PaEntry IndexSeachByName(string name)
        {
