...
namespace LookIndex
{
    public class Program
    {
        ...
        public static void Main()
        {
            ...
            IIndex<string> index_s = new GoIndex.Index<string>(path + "n_index", table.Root, en => (string)en[1], key => key.GetHashCode());
            if (build) { sw.Restart(); index_s.Build(); sw.Stop(); Console.WriteLine("biuld " + sw.ElapsedMilliseconds); }
            if (build) { sw.Restart(); ((GoIndex.Index<string>) index_s).Build0(); sw.Stop(); Console.WriteLine("biuld0 " + sw.ElapsedMilliseconds); }
           ...
           index_s = new GoIndex.Index<string>(path + "n_index2", table.Root, en => (string)en[1], null /* key => key.GetHashCode()*/);
           if (build) { sw.Restart(); index_s.Build(); sw.Stop(); Console.WriteLine("biuld " + sw.ElapsedMilliseconds); }
           RunTest<string>(index_s, row => row[1].ToString(), (maxCount / 2).ToString(), () => rnd.Next(maxCount * 2).ToString());


                Console.WriteLine("start int key GoIndex");
            IIndex<int> index = new GoIndex.Index<int>(path + "n_index_ints1", table.Root, en => (int)en[2], null /* key => key.GetHashCode()*/);
           if (build) { sw.Restart(); index_s.Build(); sw.Stop(); Console.WriteLine("biuld " + sw.ElapsedMilliseconds); }
