...
namespace PolarTableIndex
{
    public class IndexInsideRecursive  :PaCell  
    {
        ...
        public static IndexInsideRecursive OpenIndex(string path, string name, Type[] types)
        {
            var spoDir = new DirectoryInfo(path + "\\"+ name);
            if (!spoDir.Exists) spoDir.Create();    
            return new IndexInsideRecursive(spoDir, types, 1000);
        }
